...
function getAllPossibleCoordsKing(cellCoord) {
    ...
    for (var i = cellCoord.i - 1; i <= cellCoord.i + 1; i++) {
        if (i < 0 || i > 8 - 1) continue
        for (var j = cellCoord.j - 1; j <= cellCoord.j + 1; j++) {

            if (j < 0 || j > 8 - 1) continue
...
function getAllPossibleCoordsQueen(cellCoord) {
    ...
    var i = cellCoord.i - 1;
    for (var idx = cellCoord.j + 1; i >= 0 && idx < 8; idx++) {
        var coord = { i: i--, j: idx };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
    var jdx = cellCoord.j - 1;
    for (i = cellCoord.i - 1; i >= 0 && jdx >= 0; i--) {
        coord = { i: i, j: jdx-- };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
    ...
    var jdx = cellCoord.j + 1;
    for (i = cellCoord.i + 1; i < 8 && jdx < 8; i++) {
        coord = { i: i, j: jdx++ };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
...
function getAllPossibleCoordsRook(cellCoord) {
    var res = [];
    for (var i = 0; i < gBoard.length; i++) {
        for (var j = 0; j < gBoard[0].length; j++) {
            if (i === cellCoord.i || j === cellCoord.j) {
    ...
    var jdx = cellCoord.j - 1
    for (var i = cellCoord.i + 1; i < gBoard.length && jdx >= 0; i++) {
        coord = { i: i, j: jdx-- };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
...
function getAllPossibleCoordsBishop(cellCoord) {
    var res = [];
    var i = cellCoord.i - 1;
    for (var idx = cellCoord.j + 1; i >= 0 && idx < 8; idx++) {
        var coord = { i: i--, j: idx };
